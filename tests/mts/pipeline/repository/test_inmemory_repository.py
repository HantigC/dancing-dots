...
from mts.pipeline.repository.base import (
    DEFAULT_SCENE,
...
def test_add_image_is_idempotent_for_same_filepath(repo):
    id_a = repo.add_image("a.jpg")
    id_a_again = repo.add_image("a.jpg")
...
def test_add_metadata_raises_on_duplicate_key(repo):
    img_id = repo.add_image("a.jpg")
...
def test_add_pairs_and_get_pairs(repo):
    ...
    assert set(repo.get_pairs()) == {(id_a, id_b), (id_b, id_c)}
...
def test_get_pairs_filters_by_scene(repo):
    ...
    assert repo.get_pairs(scene="scene_a") == [(id_a, id_b)]
    assert repo.get_pairs(scene="scene_b") == [(id_c, id_d)]
    assert set(repo.get_pairs()) == {(id_a, id_b), (id_c, id_d)}
